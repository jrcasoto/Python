...
WORDLIST_FILENAME = "G:\Meu Drive\Coding Repository\Courses\Python\MIT_6.001x\Problem Sets\pset4\words.txt"
...
def calculateHandlen(hand):
    ...
    returns: integer
    """
    return sum([x for x in list(hand.values())])
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    while calculateHandlen(hand) > 0: # As long as there are still letters left in the hand:
        ...
        if word == '.': # If the input is a single period:
            print('Goodbye! Total score: ', score, 'points.') # Game is over (user entered a '.' or ran out of letters), so tell user the total score
    ...
    if calculateHandlen(hand) == 0:
        print('Run out of letters. Total score:', score, 'points.') # Game is over (user entered a '.' or ran out of letters), so tell user the total score
...
def playGame(wordList):
    ...
    while option != 'e': # Exit game
        option = input('Enter n to deal a new hand, r to replay the last hand, or e to end game: ')
        if option == 'r': # Replay
            if len(hand) == 0: # User never started a game; raise expection
                print('You have not played a hand yet. Please play a new hand first!\n', end="\n")
            else:
                playHand(hand, wordList, n)
        elif option == 'n': # New game
            hand = dealHand(n)
            playHand(hand, wordList, n)
        elif option == 'e':
            break # Exit game
